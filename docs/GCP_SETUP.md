...
- Access to the GCP project (e.g., `rle-guate`)
...
```bash
gcloud projects add-iam-policy-binding "rle-guate" \
  --member="serviceAccount:github-actions-rle@rle-guate.iam.gserviceaccount.com" \
...
```bash
gcloud projects describe rle-guate --format="value(projectNumber)"
```
...
gcloud iam service-accounts add-iam-policy-binding \
  "github-actions-rle@rle-guate.iam.gserviceaccount.com" \
  --project="rle-guate" \
...
| `GCP_SERVICE_ACCOUNT` | `github-actions-rle@rle-guate.iam.gserviceaccount.com` |
...
```
Caller does not have required permission to use project rle-guate.
